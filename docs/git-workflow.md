...
**판단 기준**: 커밋 설명에 "그리고"가 필요하면 쪼개야 한다는 신호다(Linux 커널 커밋 가이드라인, Pro Git 공통 원칙). "가능한 한 작게, 근데 그 자체로 완결되고 동작하는 단위"까지가 목표 — 한 줄 단위로 쪼개란 뜻은 아니다.

**왜(bisect가 실질적 이유)**: 버그가 생기면 `git bisect`로 원인 커밋을 이분탐색하는데, 여러 변경이 한 커밋에 뭉쳐있으면 그 커밋 안 어디가 문제인지 더 못 좁힌다. 원자적 커밋이어야 이분탐색의 해상도가 실제로 의미 있어진다.
